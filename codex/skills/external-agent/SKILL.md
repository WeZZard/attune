---
name: external-agent
description: Dispatch one task brief to the best-fit external agent CLIs, per the external agents guidelines. Input is a markdown task brief (## Metadata with GOAL/TAGS/AGENTS/CAPABILITIES_MARKER, the task prompt in EXTERNAL_AGENT_TASK_PROMPT tags, ## Response with the report shape). The router probes all agent facts in one matrix call, selects by task category, verifies CLI flags against live --help, launches headless runs, and reports as the brief specifies. It performs no synthesis and no judgment.
---

<!-- GENERATED from agents/external-agent.md by utils/generate-platform-assets.sh — edit the source, then regenerate. -->
...
1. **Prepare — one Bash call.** Create a workdir and gather every fact at once:
   ...
   bash "$ATTUNE_ROOT/scripts/external-agents.sh" matrix "<CAPABILITIES_MARKER if the brief provides one, else $d/facts.json>"
   ```
...
6. **Prepare the prompt.** Write the text between the `<EXTERNAL_AGENT_TASK_PROMPT>` tags to `$d/prompt` with a quoted heredoc (delimiter lines at column 1). When the `## Response` section requires artifact paths and the task prompt does not already demand them, append this block to the prompt file:
   ...
   When the task must write into a repository, first create a worktree — `bash "$ATTUNE_ROOT/scripts/worktree.sh" create <repo-dir> <name>` — and run the agent with the worktree as its working directory.
